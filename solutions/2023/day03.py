...
def touches_symbol(symbols, x, y, length):
    """
...
def get_num_at(data, numbers, x, y):
    """
    Given a position in the grid, return the number that it is part of as well as the starting
    position, otherwise None
    """
    if not (valid(data, x, y) and data[y][x].isdigit()):
        return None, None
    while valid(data, x - 1, y) and data[y][x-1].isdigit():
        x -=1  # go to front of number
    return (x, y), numbers[x, y]
        
def get_surrounding_numbers(data, numbers, x, y) -> list[int]:
    """
    ...
    for dy in (y-1, y, y+1):
        for dx in (x-1, x, x+1):
            ...
            num_pos, num = get_num_at(data, numbers, dx, dy)
            if num is not None:
                nums[num_pos] = num
...
def main(data):
    ...
    for (x, y), number in numbers.items():
        if touches_symbol(symbols, x, y, len(number)):
            total1 += int(number)

    total2 = 0
    for (x, y), symbol in symbols.items():
        if symbol == "*":
            s = get_surrounding_numbers(data, numbers, x, y)
            if len(s) == 2:
